          ...
          <v-card>
            <!-- Header -->
              <v-toolbar color="amber darken-3">
                ...
                <v-toolbar-title class="hidden-md-and-down">Genealogy</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-flex xs7 md4>
                    <v-text-field color="white" id="search" type="text" label="Search ID" prepend-inner-icon="search"/>
                </v-flex>
                            ...
                            <v-card flat>
                                ...
                                <v-card-text>
                                    <v-form>
                                        <v-container grid-list-sm bt-0>
                                            ...
                                            <v-layout row wrap>
                                                <v-flex xs12 md12>
                                                    <v-layout row wrap>
                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small  id="lname" type="text" label="Last Name" v-model="lastname" required autofocus prepend-inner-icon="account_circle"/>
                                                        </v-flex>
                                                        <v-flex xs12 md5>
                                                            <v-text-field outline small  id="fname" type="text" label="First Name" v-model="firstname" required />
                                                        </v-flex>
                                                        <v-flex xs12 md1>
                                                            <v-text-field outline small  id="mi" type="text" label="M.I" v-model="mi" />
                                                        </v-flex>
                                                    </v-layout>
                                                </v-flex>
                                                <!-- Password -->
                                                <v-flex xs12 md12>
                                                    <v-text-field outline small   id="password" :type="show1 ? 'text' : 'password'" @click:append="show1 = !show1" :append-icon="show1 ? 'visibility' : 'visibility_off'" label="Password" v-model="password" required prepend-inner-icon="lock"/>
                                                </v-flex>
                                                <!-- Confirm Password -->
                                                <v-flex xs12 md12>
                                                    <v-text-field outline small   id="password-confirm" :type="show2 ? 'text' : 'password'" @click:append="show2 = !show2" :append-icon="show2 ? 'visibility' : 'visibility_off'" label="Confirm Password" v-model="password_confirmation" required prepend-inner-icon="lock"/>
                                                </v-flex>
                                                <!-- Address -->
                                                <v-flex xs12 md12>
                                                    <v-text-field outline small   id="address" type="text" label="Address" v-model="address" required prepend-inner-icon="location_on"/>
                                                </v-flex>
                                                <!-- Email and Contact -->
                                                <v-flex xs12 md12>
                                                    <v-layout row wrap>
                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small   id="email" type="email" label="Email Address" v-model="email" required prepend-inner-icon="alternate_email"/>
                                                        </v-flex>

                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small   id="contactno" type="text" label="Contact Number" v-model="contact" required prepend-inner-icon="phone" maxlength="15" onkeypress='return event.charCode >= 48 && event.charCode <= 57'/>
                                                        </v-flex>
                                                    </v-layout>
                                                </v-flex>
                                                ...
                                                <v-flex xs12 md12>
                                                    <v-layout row wrap>
                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small   id="sponsor" type="text" label="Sponsor" v-model="sponsor" required disabled prepend-inner-icon="group_add"/>
                                                        </v-flex>
                                                        
                                                        <v-flex xs12 md6>
                                                            <v-dialog ref="dialogfrom" v-model="modal" :return-value.sync="birthdate" persistent lazy full-width width="290px">
                                                                <template v-slot:activator="{ on }">
                                                                    <v-text-field outline small   v-model="birthdate" label="Birthdate" prepend-inner-icon="event" v-on="on"></v-text-field>
                                                                </template>
                                                ...
                                                <v-flex xs12 md12>
                                                    <v-layout row wrap>
                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small   id="placement" type="text" label="Placement-ID" v-model="placement" disabled required prepend-inner-icon="device_hub"/>
                                                        </v-flex>

                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small   id="position" type="text" label="Position" v-model="position" disabled required prepend-inner-icon="code"/>
                                                                <!-- <v-select :items="items" label="Outline small  style" outline small   required prepend-inner-icon="code"></v-select> -->
                                                        </v-flex>
                                                    </v-layout>
                                                </v-flex>
                                                <v-flex xs12 md12>
                                                    <v-layout row wrap>
                                                        <!-- Activation Code -->
                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small  id="activationcode" type="text" label="Activitaion Code" v-model="activationcode" required prepend-inner-icon="dialpad"/>
                                                        </v-flex>
                                                       <!-- Pin Code -->
                                                        <v-flex xs12 md6>
                                                            <v-text-field outline small  id="pin" type="text" label="Pin Code" v-model="pin" required prepend-inner-icon="dialpad"/>
                                                        </v-flex>
                                                    </v-layout>
                                                </v-flex>
                                            </v-layout>
                                        ...
                                        <v-flex xs12 md12>
                                            <v-btn large round outline small  type="submit" color="amber darken-3" @click.prevent="registerBronze"> Register</v-btn>
                                        </v-flex>
                                    </v-form>
                                </v-card-text>
                            </v-card>
                        ...
                        <v-card-text>
                            <v-form>
                                <v-container grid-list-sm bt-0>
                                    <!-- Pin Code -->
                                    <v-flex xs12 md12>
                                        <v-text-field outline small   id="sponsor" type="text" label="Pin Code" v-model="pin" required prepend-inner-icon="dialpad"/>
                                    </v-flex>
                                     <!-- Activation Code -->
                                    <v-flex xs12 md12>
                                        <v-text-field outline small   id="sponsor" type="text" label="Activitaion Code" v-model="activationcode" required prepend-inner-icon="dialpad"/>
                                    </v-flex>
                                     <!-- Sponsor ID -->
                                    <v-flex xs12 md12>
                                        <v-text-field outline small  id="sponsor" type="text" label="Sponsor" v-model="sponsor" disabled required prepend-inner-icon="group_add"/>
                                    </v-flex>
                                    <!-- Placement ID and Position -->
                                    <v-flex xs12 md12>
                                        <v-layout row wrap>
                                            <v-flex xs12 md6>
                                                <v-text-field outline small  id="placement" type="text" label="Placement-ID" v-model="email" disabled required prepend-inner-icon="device_hub"/>
                                            </v-flex>

                                            <v-flex xs12 md6>
                                                <v-select outline small  id="position" label="Position" :items="items" disabled required prepend-inner-icon="code"/>
                                            </v-flex>
                                        </v-layout>
                                    </v-flex>

                                    <v-flex xs12 md12>
                                        <v-btn large round outline small  type="submit" color="amber darken-3">Unlock Bronze</v-btn>
                                    </v-flex>
                                </v-container>
                            </v-form>
                        </v-card-text>
...
<script>
  export default {
    data: () => ({
        ...
        Bronze: true, //Temporary Variable for Bronze Dapat account_type este
        items: ['Left', 'Right'], //Select in Unlocking Bronze
        ProfileDialog: false, //Personal Information Dialog
        NewDialog: false, //For Adding New Genea
        current_genea: sessionStorage.getItem('id'), //Id of the current genealogy
    ...
    computed: {
      ...
      current(){
        if (this.current_genea == sessionStorage.getItem('id')){
            return false
        }
        else{
            return true
    ...
    methods:{
        ...
        getReferalTree(){
            ...
            this.current_genea = this.genealogiesInfo.reference;
